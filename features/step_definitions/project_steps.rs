...
pub fn register_steps(c: &mut CucumberRegistrar<CucumberWorld>) {
  Given!(c; "^a project$", |_, world: &mut CucumberWorld, _| {
    match fs::create_project() {
      ...
      Err(ref err) => InvokeResponse::with_fail_message(format!("Failed to create project {}", err))
    }
  });
  ...
  Given!(c; "^the steps$", |_, world: &mut CucumberWorld, (step_set,): (String,)| {
    match world.current_project {
      None => return InvokeResponse::with_fail_message("There was no project to add steps to"),
  ...
  Then!(c; "^the project compiles$", |_, world: &mut CucumberWorld, _| {
    match world.current_project {
      None => return InvokeResponse::with_fail_message("There was no project to compile"),
      Some(ref mut project) => {
        match project.compile() {
          Err(err) => InvokeResponse::with_fail_message(format!("The project failed to compile with: {}", err)),
  ...
  Then!(c; "^the feature passes$", |_, world: &mut CucumberWorld, _| {
    match world.execute_result {
      ...
      Some(Err(ref err)) => InvokeResponse::with_fail_message(&format!("Expected scenario to pass but it failed with {}", err)),
  ...
  Then!(c; "^the feature fails with \"([^\"]*)\"$", |_, world: &mut CucumberWorld, (fail_msg,): (String,)| {
    match world.execute_result {
      None => InvokeResponse::with_fail_message("Expected there to be an execute result but there wasn't one"),
      Some(Ok(_)) => InvokeResponse::with_fail_message("Expected scenario to fail but it passed"),
      Some(Err(ref err)) => {
  ...
  Then!(c; "^the feature reports a pending step$", |_, world: &mut CucumberWorld, _| {
    match world.execute_result {
      None => InvokeResponse::with_fail_message("Expected there to be an execute result but there wasn't one"),
      Some(Err(_)) => InvokeResponse::with_fail_message("Expected scenario to pass (to retrieve a pending step) but it failed"),
